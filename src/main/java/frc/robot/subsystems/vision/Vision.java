...
public class Vision extends SubsystemBase {
  ...
  public void periodic() {
    ...
    for(poseObservation poseObservation : poseObservations){
      ...
      if(Math.abs(poseAtTime.getRotation().minus(poseObservation.pose.toPose2d().getRotation()).getRadians()) 
        > VisionConstants.yawTolerance){
          continue;
      }

      drive.addVisionMeasurement(poseObservation.pose.toPose2d(), num_cameras, poseObservation.stdDevs);
    }
  }
