...
public class upperFeedFoward {
    ...
    public double[] toMotorTorque(double armCurrentAngle, double elevatorCurrentHeight,
                                double elevatorAcc, double armAngularVelocity, double armAngularAcc){
        double[] position = {
            Math.cos(armCurrentAngle),
            Math.sin(armCurrentAngle) + elevatorCurrentHeight
        };
        double armMotorTorque = ((arm.armMass + intake.intakeMass)
                          * (g + elevatorAcc)
                          * (position[0] * arm.armCOMRadius / arm.armLength)
                          + (arm.armMass / 3 + intake.intakeMass)
                          * (arm.armLength * arm.armLength) * armAngularAcc
                          );
        double liftMotorTorque = ((elevator.elevatorMass
                           * (g + elevatorAcc)
                           + (arm.armMass * arm.armCOMRadius * armAngularVelocity * armAngularVelocity
                              + intake.intakeMass * arm.armLength * (armAngularVelocity * armAngularVelocity))
                           * (position[1] - elevator.elevatorHeight) / arm.armLength
                           )
