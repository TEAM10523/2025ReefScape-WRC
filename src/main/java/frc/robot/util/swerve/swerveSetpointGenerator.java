...
public class swerveSetpointGenerator {
  ...
  public vector getAxisRadius(vector v, double omega) {
    return new vector(new double[] {(-v.vectorArray[1]) / (omega), (v.vectorArray[0]) / (omega)});
  }
  ...
  public double getOmega(vector r, vector v) {
    return new vector(new double[] {-r.vectorArray[1], r.vectorArray[0]}).dot(v);
  }

  private double getModuleAccForce(
      double v, double vmax, double StallForce, double ForceWithoutSlip, double ForceVoltageLim) {
    double tempF = Math.min((StallForce * (1 - (v / vmax))), ForceWithoutSlip);
    return Math.min(tempF, ForceVoltageLim);
  }

  private double getModuleDecForce(
      double v, double vmax, double breakingForce, double ForceWithoutSlip) {
    double tempF = Math.min((breakingForce * (v / vmax)), ForceWithoutSlip);
    return tempF;
  }
    ...
    for (int i = 0; i < modulesForce.length; i++) {
      torque +=
          modulesForce[i].dot(
              new vector(
                  new double[] {
                    -modulePositions[i].vectorArray[1], modulePositions[i].vectorArray[0]
                  }));
    }
    return torque;
  }
  ...
  public swerveSetpoint generateSetpoint(
      final ChassisLimits LimitConstants,
    ...
    SwerveModuleState[] wpiSwerveModuleStates = cuKinematic.toSwerveModuleStates(desireState);
    ...
    for (int i = 0; i < num_modules; i++) {
      ...
      maxDesireSpeed =
          Math.max(maxDesireSpeed, Math.abs(wpiSwerveModuleStates[i].speedMetersPerSecond));
    }

    double desaturateScale =
        maxDesireSpeed > LimitConstants.MaxDriveVelocity
            ? LimitConstants.MaxDriveVelocity / maxDesireSpeed
            : 1;
    ...
    for (int i = 0; i < num_modules; ++i) {
      // double f = isAcc[i]?
      //     getModuleAccForce(modulesV[i].norm(), LimitConstants.MaxDriveVelocity,
      // LimitConstants.Fstall, maxForceWithoutSlipping, LimitConstants.FmaxWithVoltagelimit) :
      //     getModuleDecForce(modulesV[i].norm(), LimitConstants.MaxDriveVelocity,
      // LimitConstants.Fbreaking, maxForceWithoutSlipping);
      ...
      double f =
          isAcc[i]
              ? getModuleAccForce(
                  modulesV[i].norm(),
                  LimitConstants.MaxDriveVelocity,
                  LimitConstants.Fstall,
                  maxForceWithoutSlipping,
                  LimitConstants.FmaxWithVoltagelimit)
              : getModuleDecForce(
                  modulesV[i].norm(),
                  LimitConstants.MaxDriveVelocity,
                  LimitConstants.Fbreaking,
                  maxForceWithoutSlipping);
      modulesDv[i] = desireVs[i].add(modulesV[i].times(-1));
    ...
    double maxOmegaStep =
        dt * Math.abs(getRobotTorque(modulesForce, modulePositions)) / LimitConstants.robotMIO;
    double maxVelocityUpStep =
        dt * Math.abs(getRobotForce(modulesForce, preVelocity)) / LimitConstants.robotMass;
    ...
    double deltaVSide =
        Math.abs(
                new vector(new double[] {-preDirection.vectorArray[1], preDirection.vectorArray[0]})
                    .dot(deltaV))
            + Util.kEpsilon;
    ...
    //     new Translation2d(modulesForce[0].vectorArray[0],
    // modulesForce[0].vectorArray[1]).getAngle().getRadians(), modulesForce[0].norm(),
    //     new Translation2d(modulesForce[1].vectorArray[0],
    // modulesForce[1].vectorArray[1]).getAngle().getRadians(), modulesForce[1].norm(),
    //     new Translation2d(modulesForce[2].vectorArray[0],
    // modulesForce[2].vectorArray[1]).getAngle().getRadians(), modulesForce[2].norm(),
    //     new Translation2d(modulesForce[3].vectorArray[0],
    // modulesForce[3].vectorArray[1]).getAngle().getRadians(), modulesForce[3].norm(),
    ...
    SmartDashboard.putNumberArray(
        "currentSpeed",
        new double[] {preVelocity.vectorArray[0], preVelocity.vectorArray[1], preOmega});
    ...
    ChassisSpeeds desireChassisSpeeds =
        new ChassisSpeeds(
            desireVelocity.vectorArray[0], desireVelocity.vectorArray[1], desireOmega);
    wpiSwerveModuleStates = cuKinematic.toSwerveModuleStates(desireChassisSpeeds);
    ...
    for (int i = 0; i < num_modules; ++i) {
      final var deltaRotation =
          prevSetpoint
              ...
              .turnPosition
              .unaryMinus()
              .rotateBy(retStates[i].turnPosition);
      ...
      retStates[i].turnVelocity =
          -prevSetpoint.cuModuleState[i].turnPosition.minus(retStates[i].turnPosition).getRadians()
              / dt;
      retStates[i].driveAcceleration =
          (retStates[i].driveVelocity - prevSetpoint.cuModuleState[i].driveVelocity) / dt;
    }
