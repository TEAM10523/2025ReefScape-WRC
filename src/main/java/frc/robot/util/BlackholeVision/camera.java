...
public class camera {
    ...
    public poseObservation getRobotPose(){
        ...
        Matrix<N3, N1> stdDevs = new Matrix<>(VecBuilder.fill(error * 100, error * 100, error * 1000));
        if(error < 0){
            stdDevs = null;
        }
        ...
        return new poseObservation(new Pose3d(
            poseList[0], poseList[1], poseList[2],
            new Rotation3d(poseList[3], poseList[4], poseList[5])
        ), stdDevs, FPGATimestamp);
    }
